...
// Run audio on Core 0. Core 1 (the Arduino/LVGL loop + display SPI-DMA) caused
// CONSTANT distortion - the audio task fights the display renderer continuously.
// Core 0 is clean in steady state; its only artifact is a brief (~10s) startup
// distortion while the WiFi stack (also Core 0) is connecting, which the larger
// I2S DMA buffer (see i2s_audio.h) is meant to ride out.
