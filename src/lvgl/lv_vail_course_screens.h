...
static void vail_course_module_click_handler(lv_event_t* e) {
    ...
    // Locked modules are visible and navigable but not enterable.
...
/*
 * Module selection: a scrollable vertical list of all 12 modules. Every row is
 * navigable (UP/DOWN); locked rows are dimmed and do nothing on ENTER. The
 * focused row scrolls into view, so the list always fits regardless of count.
 */
