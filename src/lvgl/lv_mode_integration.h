...
lv_obj_t* createScreenForModeInt(int mode) {
    // Vail CW School hub + practice modes (handled before the menu switch)
    lv_obj_t* schoolScreen = createSchoolScreenForMode(mode);
    if (schoolScreen != NULL) return schoolScreen;
...
void initializeModeInt(int mode) {
    ...
    switch (mode) {
        ...
        case MODE_PRACTICE:
            ...
            break;
        case MODE_SCHOOL_SEND:
            Serial.println("[ModeInit] Starting Send Practice (school)");
...
bool handleGlobalHotkey(char key) {
    ...
    if ((key == 'V' || key == 'v') && isPureNavigationMenuInt(currentModeInt)) {
        Serial.printf("[Hotkey] V pressed in navigation menu mode %d, opening Quick Settings\n", currentModeInt);
        openQuickSettings();
        return true;
    }
...
void onLVGLMenuSelect(int target_mode) {
    ...
    if (target_mode == MODE_FACTORY_RESET) {
        playAlertChirp();
        createConfirmDialog("Factory Reset",
            "Erase ALL settings, progress,\nand WiFi credentials?\n"
            "This cannot be undone.",
            factoryResetConfirmCb, nullptr);
        return;
    }
...
int getParentModeInt(int mode) {
    ...
    // Special case: mode launched directly from the home dashboard returns to
    // the dashboard rather than its legacy menu parent. Armed in
    // homeLauncherClickHandler (lv_home_screen.h), consumed once here.
    if (mode == homeLaunchTarget) {
        homeLaunchTarget = -1;
        Serial.printf("[ModeIntegration] Returning from home-launched mode %d to HOME\n", mode);
        return MODE_HOME;
    }
...
static const ModeCallbackEntry cleanupTable[] = {
    ...
    { MODE_SCHOOL_COPY,                  cleanupSchoolPractice },
...
void showInitialLVGLScreen() {
    ...
    if (home != NULL) {
        ...
        setCurrentModeFromInt(MODE_HOME);
        currentSelection = 0;

        // Debug: verify navigation group has widgets
        lv_group_t* group = getLVGLInputGroup();
        Serial.printf("[ModeIntegration] Home dashboard loaded, nav group has %d objects\n",
                      group ? lv_group_get_obj_count(group) : -1);
    } else {
        Serial.println("[ModeIntegration] CRITICAL: Failed to create home screen!");
    }
}
