...
// Watchdog: reboot if loop() stalls this long (genuine hang/deadlock). Generous
// enough to tolerate blocking network ops (which have ~10-15s internal timeouts)
// and the chunked web-files download (polled per loop), so only a real hang trips it.
...
#include "src/lvgl/lv_home_screen.h"
#include "src/lvgl/lv_school_screens.h"
#include "src/lvgl/lv_quick_settings.h"
...
void setup() {
  ...
  // Log why the device last reset - useful for field diagnostics (a watchdog,
  // panic, or brownout reset points at a different problem than a clean power-on).
  switch (esp_reset_reason()) {
    case ESP_RST_POWERON:  Serial.println("Reset reason: power-on"); break;
    case ESP_RST_SW:       Serial.println("Reset reason: software restart"); break;
    case ESP_RST_PANIC:    Serial.println("Reset reason: PANIC/exception (crash)"); break;
    case ESP_RST_TASK_WDT: Serial.println("Reset reason: TASK WATCHDOG (loop hang)"); break;
    case ESP_RST_INT_WDT:  Serial.println("Reset reason: interrupt watchdog"); break;
    case ESP_RST_WDT:      Serial.println("Reset reason: other watchdog"); break;
    case ESP_RST_BROWNOUT: Serial.println("Reset reason: BROWNOUT (low voltage)"); break;
    case ESP_RST_DEEPSLEEP: Serial.println("Reset reason: deep sleep wake"); break;
    default:               Serial.println("Reset reason: other"); break;
  }
  ...
  // normal UI won't load. No-op unless both paddles are held at this moment.
  checkFactoryResetGesture(DIT_PIN, DAH_PIN, PADDLE_ACTIVE);
  ...
  // Enable the task watchdog on the loop task now that init is fully done (init
  ...
  // Startup test beeps removed - they were jarring on boot. I2S audio is
  // exercised the first time a tone/morse plays in the UI.
...
void pollSchoolSend() { schoolSendPoll(); }
...
void loop() {
  ...
  // Feed the loop watchdog. If we never get back here within LOOP_WDT_TIMEOUT_S
  // (a hang/deadlock somewhere below), the WDT reboots the device.
  esp_task_wdt_reset();
  ...
  // Keep the clock synced once WiFi is up (non-blocking), so QSO timestamps are
  // correct even if WiFi connected after boot.
  ensureNTPStarted();
